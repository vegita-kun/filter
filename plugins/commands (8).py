...
async def start(client, message):
    try: 
        ...
        if len(message.command) != 2:
            ...
            reply_markup = InlineKeyboardMarkup(buttons)
            m=await message.reply_sticker("CAACAgQAAxkBAAIH52geXZwKM_EU6DFulu3CClLGrfpPAAJ9CgACv9i5UQP6CfFALqk1HgQ") 
...
async def gen_sbatch_links(bot, message):
    ...
    # Fetch media
    files = []
    async for msg in bot.iter_messages(f_chat_id, min_id=min(f_msg_id, l_msg_id)-1, max_id=max(f_msg_id, l_msg_id)+1):
    ...
    if not files:
        return await message.reply("No media found in given range.")

    # Save to temp keyword
    keyword = f"sbatch_{message.from_user.id}_{''.join(random.choices(string.ascii_lowercase, k=5))}"
    temp.GETALL[keyword] = files

    # Trigger filter-style message
    await message.reply(
        f"✅ Batch created with `{len(files)}` files.\nClick below to view results:",
        reply_markup=InlineKeyboardMarkup([[
